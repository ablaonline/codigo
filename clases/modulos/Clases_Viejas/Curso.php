...
class Curso {
    ...
    public function __construct($id = NULL) {
        ...
        $consulta = $sql->filaEnObjeto($sql->seleccionar(array('cursos'), array('registros' => 'COUNT(id)')));
        $this->registros = $consulta->registros;

        $consulta = $sql->filaEnObjeto($sql->seleccionar(array('cursos'), array('registros' => 'COUNT(id)'), 'activo = "1"'));
...
//codigo para ingresar en la tabla permisos_item los perfiles con los cuales se comparte el BLOG
    ...
    public function eliminar() {
        ...
        $consulta = $sql->eliminar('foros', 'id_modulo = "' . $this->idModulo . '" AND id_registro = "' . $this->id . '"');

        /* Elimino los seguidores que tenga relacionado el curso */
        $consulta = $sql->eliminar('cursos_seguidos', ' id_curso= "' . $this->id . '"');
        ...
        if (sizeof($arregloCursos) > 0) {
            ...
            foreach ($arregloCursos as $elemento) {
                ...
                if (isset($sesion_usuarioSesion) && ($sesion_usuarioSesion->idTipo == 0 || $sesion_usuarioSesion->id == $elemento->idAutor)) {
                    $botones = '';
                    $botones .= HTML::botonModificarItem($elemento->id, $curso->urlBase);
                    $botones .= HTML::botonEliminarItem($elemento->id, $curso->urlBase);
