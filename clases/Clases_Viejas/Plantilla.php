...
//        $formaBuscador .= HTML::boton('buscar', $textos->id('BUSCAR'), 'directo');
    ...
    protected static function cargarMenus() {
        ...
        while ($menu = $sql->filaEnObjeto($menus)) {
            ...
            if ($menu->destino) {
                $listaInicio[] = HTML::enlace($menu->nombre, $menu->destino) . HTML::lista($subMenu, 'subMenu', 'ui-state-default'); //MENU PRINCIPAL
            } else {
        ...
        $enlaces .= HTML::lista($listaInicio, '', '', 'menuInicio');
        $enlaces .= HTML::lista($listaMenu, '', 'menuInicio', '');
    ...
    protected static function cargarUsuarioSesion() {
        ...
        if (!isset($sesion_usuarioSesion)) {
            ...
            $formaUsuarioExistente = HTML::contenedor($formaUsuarioExistente, 'oculto', 'contenedorCamposLogin');


            $textoANDayahoculto .= HTML::contenedor($ayah->getPublisherHTML(), 'oculto');
            ...
            self::$etiquetas['BLOQUE_USUARIO'] = HTML::bloque('datosUsuarioActual', $sesion_usuarioSesion->sobrenombre, $contenido, 'centrado', NULL, '-DS'); //Aqui debo cambiar creo...
            ...
            self::$etiquetas['BLOQUE_USUARIO'] .= /* $imagen. */HTML::acordeon(array_keys($lista), array_values($lista), 'menuOpcionesUsuario', 'margenInferior izquierda', '', '', '', $opciones);
        }
    }
    ...
    protected static function cargarCalendarioEventos() {
        ...
        $calendario = HTML::contenedor($calendario);
