...
/**
 * Efectuar la carga de los archivos contenidos en el directorio de configuración
 */
...
$ruta = $configuracion["RUTAS"]["clasesBasicas"];
require_once $ruta;

$ruta = $configuracion["RUTAS"]["clasesModulos"];
require_once $ruta;
 ...
 * Efectuar la carga de los archivos de definición de clases básicas
*/
if ($directorio = opendir($configuracion['RUTAS']['clasesEnDesarrollo'])) {

    while (false !== ($archivo = readdir($directorio))) {

        if (($archivo != '.') && ($archivo != '..') && (substr($archivo, -4) == '.php')) {
            $ruta = $configuracion['RUTAS']['clasesEnDesarrollo'].'/'.$archivo;
            require_once $ruta;
        }
...
$ruta = $configuracion["RUTAS"]["fuentes"];
require_once $ruta;
