...
function adicionarComentario($datos = array()) {
    ...
    $destino      = '/ajax/home/addComment';
    $respuesta    = array();

    if (empty($datos)) {
        $codigo  = HTML::campoOculto('procesar', 'true');
        $codigo .= HTML::campoOculto('datos[idModulo]', htmlspecialchars($forma_idModulo));
        $codigo .= HTML::campoOculto('datos[idRegistro]', htmlspecialchars($forma_idRegistro));
        $codigo .= HTML::parrafo($textos->id('COMENTARIO'), 'negrilla margenSuperior');
        $codigo .= HTML::areaTexto('datos[contenido]', 5, 50, '', '', 'txtAreaLimitado300');
...
function eliminarComentario($id, $confirmado) {
    ...
    $destino      = '/ajax/home/deleteComment';
    $respuesta    = array();

    if (!$confirmado) {
        $nombre  = HTML::frase($comentario->autor, 'negrilla');
        $nombre  = str_replace('%1', $nombre, $textos->id('CONFIRMAR_ELIMINACION_COMENTARIO'));
        $codigo  = HTML::campoOculto('procesar', 'true');
...
function adicionarVideo($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::campoTexto('datos[titulo]', 30, 50);
        $codigo .= HTML::parrafo($textos->id('DESCRIPCION'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[descripcion]', 40, 120);
        $codigo .= HTML::parrafo($textos->id('ENLACE_AL_VIDEO'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[enlace]', 50, 255, '', 'txtYoutube', '', array('ayuda' => $textos->id('AYUDA_FORMATO_ENLACE_VIDEO')));
        ...
        } elseif ( !preg_match("/\byoutube\b/i", $datos['enlace'])  ) {
...
function eliminarVideo($id, $confirmado) {
    ...
    $video        = new Video($id);
    $destino      = '/ajax/home/deleteVideo';
    $respuesta    = array();

    if (!$confirmado) {
        $nombre  = HTML::frase($video->descripcion, 'negrilla');
        $nombre  = str_replace('%1', $nombre, $textos->id('CONFIRMAR_ELIMINACION_VIDEO'));
        $codigo  = HTML::campoOculto('procesar', 'true');
        $codigo .= HTML::campoOculto('id', $id);
        $codigo .= HTML::parrafo($nombre);
        $codigo .= HTML::parrafo(HTML::boton('chequeo', $textos->id('ACEPTAR')), 'margenSuperior');
...
function eliminarVideoUsuario($id, $confirmado) {
    ...
    $video        = new Video($id);
    $destino      = '/ajax/home/deleteNewVideo';
    $respuesta    = array();
...
function adicionarImagen($datos = array()) {
    global $sql, $textos, $configuracion, $archivo_recurso, $forma_idRegistro, $forma_modulo;

    $destino      = '/ajax/home/addImage';
    $respuesta    = array();
    ...
    if (empty($datos)) {
        ...
        if($forma_modulo == 'CURSOS'){
            $codigo .= HTML::parrafo(HTML::campoChequeo('datos[notificar_estudiantes]', true).$textos->id('NOTIFICAR_ESTUDIANTES'), 'margenSuperior');
        } elseif($forma_modulo == 'USUARIOS'){
            $codigo .= HTML::parrafo(HTML::campoChequeo('datos[publicar_en_muro]', true).$textos->id('PUBLICAR_EN_MURO'), 'margenSuperior');
        }        
...
function eliminarImagen($id, $confirmado) {
    ...
    $destino      = '/ajax/home/deleteImage';
    $respuesta    = array();

    if (!$confirmado) {
        $nombre  = HTML::frase($imagen->descripcion, 'negrilla');
        $nombre  = str_replace('%1', $nombre, $textos->id('CONFIRMAR_ELIMINACION_IMAGEN'));
        $codigo  = HTML::campoOculto('procesar', 'true');
        $codigo .= HTML::campoOculto('id', $id);
        $codigo .= HTML::parrafo($nombre);
        $codigo .= HTML::parrafo(HTML::boton('chequeo', $textos->id('ACEPTAR'), 'botonOk', 'botonOk', 'botonOk'), 'margenSuperior');
...
function eliminarNotificacion($id, $confirmado) {
    ...
    $destino      = '/ajax/home/deleteNotification';
    $respuesta    = array();
...
function estoyConectado() {
    ...
    if (isset($sesion_usuarioSesion) && !empty($sesion_usuarioSesion->usuario)) {
        ...
        $codigo1     = Contacto::usuariosConectados();
        $numAmigos1  = Contacto::cantidadUsuariosConectados();
	...
	$respuesta['numUsuariosConectados'] = $numAmigos1;
	$respuesta['contenido1']           = $codigo1;
        $respuesta['destino']             = '#bloqueContactosConectados_' . $sesion_usuarioSesion->id;
	$respuesta['destino1']             = '#bloqueUsuariosConectados_' . $sesion_usuarioSesion->id;
...
function cargarFechasEventos($inicio, $fin){
    ...
    $condicion = '  UNIX_TIMESTAMP(fecha_inicio) BETWEEN "'.$inicio.'" AND "'.$fin.'" ';
    if ($idTipo != 99){        
        if($idTipo != 0){
