...
function adicionarContacto($id, $confirmado) {
    ...
    if (!$confirmado) {
        ...
        if ($consulta) {
            ...
            Servidor::notificar($id, $notificacion, array(), '1');
...
function aceptarContacto($id, $confirmado) {
    ...
    if (!$confirmado) {
        ...
        if ($consulta) {
            ...
            $mensaje          = str_replace("%1", $sesion_usuarioSesion->persona->nombreCompleto, $textos->id("TE_HA_ACEPTADO_COMO_AMIGO")); 
            Servidor::notificar($id, $mensaje, array(), '5');
                           
            $contacto_usuario = $sql->obtenerValor("usuarios", "usuario", "id = '$id'");
            $notificacion     = str_replace("%1", HTML::enlace($sesion_usuarioSesion->usuario, HTML::urlInterna("CONTACTOS", $sesion_usuarioSesion->usuario)), $textos->id("MENSAJE_ADICION_CONTACTO"));
            $notificacion     = str_replace("%2", HTML::enlace($contacto_usuario, HTML::urlInterna("CONTACTOS", $contacto_usuario)), $notificacion);
