...
function adicionarPerfil($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id("TIENE_PRIVILEGIOS_SOBRE_OTROS_PERFILES"), "negrilla margenSuperiorDoble");
        $codigo .= Perfil::mostrarChecksPerfiles();
        $codigo .= HTML::parrafo(HTML::campoChequeo("datos[visible]", true).$textos->id("VISIBLE"), "margenSuperior");
        ...
        if (empty($datos["nombre"])) {
            ...
            $respuesta["mensaje"] = $textos->id("ERROR_FALTA_SELECCIONAR_PERFIL");
...
function modificarPerfil($id, $datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id("TIENE_PRIVILEGIOS_SOBRE_OTROS_PERFILES"), "negrilla margenSuperiorDoble");
        $codigo .= Perfil::mostrarChecksPerfiles($id);
        $codigo .= HTML::parrafo(HTML::campoChequeo("datos[visible]", $perfil->visibilidad).$textos->id("VISIBLE"), "margenSuperior");
        $codigo .= HTML::parrafo(HTML::boton("chequeo", $textos->id("ACEPTAR")), "margenSuperior");
        $codigo  = HTML::forma($destino, $codigo);
