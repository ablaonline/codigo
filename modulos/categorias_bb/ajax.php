...
function cosultarItem($id) {
    ...
    if(!isset($id) || (isset($id) && !$sql->existeItem("categorias_bb", "id", $id))){
        $respuesta             = array();
    ...
    $codigo .= HTML::parrafo($textos->id("COLOR"), "negrilla margenSuperior");
    $codigo .= HTML::parrafo($objeto->color, "", "");    
...
function adicionarItem($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id('COLOR'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[color]', 20, 25, '', 'selectorColor', 'picker');        
        $codigo .= HTML::parrafo($textos->id('IMAGEN'), 'negrilla margenSuperior');
        ...
        } elseif ($area[0] != $configuracion['DIMENSIONES']['anchoCategoriaBBNormal'] || $area[1] != $configuracion['DIMENSIONES']['altoCategoriaBBNormal']) {
                    $respuesta['mensaje'] = $textos->id('ERROR_AREA_IMAGEN_CATEGORIA_BB');
        }else {
...
function modificarItem($id, $datos = array()) {
    ...
    if (!is_numeric($id) || !$sql->existeItem('categorias_bb', 'id', $id)) {
    	$respuesta['error']   = true;
    	$respuesta['mensaje'] = $textos->id('ERROR_DESCONOCIDO');
    ...
    if (empty($datos)) {
        ...
        $codigo .= HTML::parrafo($textos->id('COLOR'), 'negrilla margenSuperior');
        $codigo .= HTML::campoTexto('datos[color]', 20, 25, $objeto->color, 'selectorColor', 'picker');          
        $codigo .= HTML::parrafo($textos->id('IMAGEN'), 'negrilla margenSuperior');
        $codigo .= HTML::campoArchivo('imagen', 50, 255);
        ...
        $codigo .= Perfil::mostrarChecks($id, $objeto->idModulo);
...
function buscarItem($data) {
    ...
    $datos = $data[0];
     ...
     if(empty($datos)) { 
         ...
         $respuesta["error"]   = true;
         $respuesta["mensaje"] = str_replace("%1", "2", $textos->id("ERROR_TAMANO_CADENA_BUSQUEDA"));
...
function listarItems($cadena) {
    ...
    $respuesta = array();
    $consulta  = $sql->seleccionar(array("categorias_bb"), array("titulo"), "nombre LIKE '%$cadena%'", "", "titulo ASC", 0, 20);
