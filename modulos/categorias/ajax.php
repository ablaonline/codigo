...
function adicionarCategoria($datos = array()) {
    ...
    if (empty($datos)) {
        ...
        $codigo .= Categoria::mostrarChecksModulos($id);//pongo los checkbox con la info de los modulos
        ...
        if (empty($datos["nombre"])) {
            ...
            $respuesta["mensaje"] = $textos->id("ERROR_FALTA_SELECCIONAR_MODULOS");
...
function modificarCategoria($id, $datos = array()) {
    ...
    if (empty($datos)) {
        ...
        } elseif ($sql->existeItem("categoria", "nombre", $datos["nombre"], " id != '$id'")) {
            $respuesta["mensaje"] = str_replace("%1", $datos['nombre'], $textos->id("ERROR_EXISTE_CATEGORIA"));
